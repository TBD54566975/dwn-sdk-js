...
describe('EventLogLevel Tests', () => {
  ...
  describe('selective sync', () => {
    ...
    const addRecordsDeleteEvent = async (
      ...
      const indexes = await RecordsDeleteHandler.constructIndexes(recordsDelete, recordWrite);
      const messageCid = await Message.getCid(message);
      const watermark = await eventLog.append(author.did, messageCid, indexes);
    ...
    const initEvents = async (): Promise<string> => {
      ...
      events.set(write1, await write1.recordsWrite.constructRecordsWriteIndexes(true)); // proto1
      const write2 = await addRecordsWriteEvent(author, { schema: 'schema2', protocol: proto2, protocolPath: 'path2' });
      events.set(write2, await write2.recordsWrite.constructRecordsWriteIndexes(true)); // proto2
      ...
      events.set(deleteForWrite2, RecordsDeleteHandler.constructIndexes(deleteForWrite2.recordsDelete, write2.recordsWrite.message)); // proto2
      ...
      events.set(write3, await write3.recordsWrite.constructRecordsWriteIndexes(true)); // proto2
      const write4 = await addRecordsWriteEvent(author, { schema: 'schema2', protocol: proto1, protocolPath: 'path2' });
      events.set(write4, await write4.recordsWrite.constructRecordsWriteIndexes(true)); // proto1

      const deleteForWrite3 = await addRecordsDeleteEvent(author, write3.recordsWrite.message);
      events.set(deleteForWrite3, RecordsDeleteHandler.constructIndexes(deleteForWrite3.recordsDelete, write3.recordsWrite.message)); // proto2
