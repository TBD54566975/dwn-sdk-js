    ...
    describe("functional tests", () => {
      ...
      it("should only be able to overwrite existing record if new record has a later `messageTimestamp` value", async () => {
        ...
        const recordsWriteReply = await dwn.processMessage(
          tenant,
          recordsWriteMessageData.message,
          { dataStream: recordsWriteMessageData.dataStream }
        );
        ...
        expect(newDataEncoded).to.not.equal(
          Encoder.bytesToBase64Url(recordsWriteMessageData.dataBytes!)
        );

        const newRecordsWriteReply = await dwn.processMessage(
          tenant,
          newRecordsWrite.message,
          { dataStream: newRecordsWrite.dataStream }
        );
        expect(newRecordsWriteReply.status.code).to.equal(202);

        // verify new record has overwritten the existing record
        const newRecordsQueryReply = await dwn.processMessage(
          tenant,
          recordsQueryMessageData.message
        );
        ...
        expect(newRecordsQueryReply.entries![0].encodedData).to.equal(
          newDataEncoded
        );
        ...
        // expecting unchanged
        const thirdRecordsQueryReply = await dwn.processMessage(
          tenant,
          recordsQueryMessageData.message
        );
        expect(thirdRecordsQueryReply.status.code).to.equal(200);
        expect(thirdRecordsQueryReply.entries?.length).to.equal(1);
        expect(thirdRecordsQueryReply.entries![0].encodedData).to.equal(
          newDataEncoded
        );
      });
      ...
      it("should only be able to overwrite existing record if new message CID is larger when `messageTimestamp` value is the same", async () => {
        ...
        const originatingMessageWriteReply = await dwn.processMessage(
          ...
          { dataStream: originatingMessageData.dataStream }
        );
        ...
        const recordsWriteReply = await dwn.processMessage(
          tenant,
          olderWrite.message,
          { dataStream: olderWrite.dataStream }
        );
        expect(recordsWriteReply.status.code).to.equal(202);
        ...
        expect(recordsQueryReply.status.code).to.equal(200);
        expect(recordsQueryReply.entries?.length).to.equal(1);
        expect(recordsQueryReply.entries![0].descriptor.dataCid).to.equal(
          olderWrite.message.descriptor.dataCid
        );
        ...
        const newRecordsWriteReply = await dwn.processMessage(
          tenant,
          newerWrite.message,
          { dataStream: newerWrite.dataStream }
        );
        expect(newRecordsWriteReply.status.code).to.equal(202);
        ...
        expect(newRecordsQueryReply.status.code).to.equal(200);
        expect(newRecordsQueryReply.entries?.length).to.equal(1);
        expect(newRecordsQueryReply.entries![0].descriptor.dataCid).to.equal(
          newerWrite.message.descriptor.dataCid
        );
        ...
        expect(thirdRecordsQueryReply.status.code).to.equal(200);
        expect(thirdRecordsQueryReply.entries?.length).to.equal(1);
        expect(thirdRecordsQueryReply.entries![0].descriptor.dataCid).to.equal(
          newerWrite.message.descriptor.dataCid
        ); // expecting unchanged
      });
      ...
      it("should not allow changes to immutable properties", async () => {
        ...
        TestStubGenerator.stubDidResolver(didResolver, [
          initialWriteData.author,
        ]);

        const initialWriteReply = await dwn.processMessage(
          tenant,
          initialWriteData.message,
          { dataStream: initialWriteData.dataStream }
        );
        ...
        let reply = await dwn.processMessage(tenant, childMessageData.message, {
          dataStream: childMessageData.dataStream,
        });
        ...
        reply = await dwn.processMessage(tenant, childMessageData.message, {
          dataStream: childMessageData.dataStream,
        });
      ...
      it("should inherit data from previous RecordsWrite given a matching dataCid and dataSize and no dataStream", async () => {
        ...
        const initialWriteReply = await dwn.processMessage(tenant, message, {
          dataStream,
        });
        expect(initialWriteReply.status.code).to.equal(202);
        ...
        const readMessageReply = await dwn.processMessage(
          tenant,
          readMessage.message
        );
      ...
      it("should not allow non-initial writes to be written without supplying data", async () => {
        ...
        const { recordsWrite, dataStream, dataBytes } =
        ...
        expect(newRecordsWriteReply.status.detail).to.contain(
      ...
      it("should throw if associate returns undefined", async () => {
        ...
        sinon
          .stub(dataStore, "get")
          .withArgs(alice.did, prevMessageCid, dataCid)
          .resolves({ dataCid, dataSize } as GetResult);

        const write2Reply = await dwn.processMessage(alice.did, write2.message);
        expect(write2Reply.status.code).to.equal(400);
        expect(write2Reply.status.detail).to.contain(
      ...
      describe("owner signature tests", () => {
        it("should use `ownerSignature` for authorization when it is given - flat-space", async () => {
          ...
          const writeReply = await dwn.processMessage(bob.did, message, {
            dataStream,
          });
          ...
          const readReply = await dwn.processMessage(
            bob.did,
            recordsRead.message
          );
          ...
          const aliceWriteReply = await dwn.processMessage(
            alice.did,
            ownerSignedMessage.message,
            { dataStream: aliceDataStream }
          );
          expect(aliceWriteReply.status.code).to.equal(202);

          // Test that Bob's message can be read from Alice's DWN
          const readReply2 = await dwn.processMessage(
            alice.did,
            recordsRead.message
          );
          ...
          const dataFetched = await DataStream.toBytes(
            readReply2.record!.data!
          );
          expect(ArrayUtility.byteArraysEqual(dataFetched, dataBytes!)).to.be
            .true;
        });
        ...
        it("should use `ownerSignature` for authorization when it is given - protocol-space", async () => {
          ...
          const recordsWriteReply = await dwn.processMessage(
            ...
            { dataStream: bobRecordsWrite.dataStream }
          );
          ...
          // Alice augments Bob's message as an external owner
          const ownerSignedMessage = await RecordsWrite.parse(
            bobRecordsWrite.message
          );
          await ownerSignedMessage.signAsOwner(Jws.createSigner(alice));

          // Test that Alice can successfully retain/write Bob's message to her DWN
          const aliceDataStream = DataStream.fromBytes(
            bobRecordsWrite.dataBytes!
          );
          const aliceWriteReply = await dwn.processMessage(
            alice.did,
            ownerSignedMessage.message,
        ...
        it("should throw if `ownerSignature` fails verification", async () => {
          ...
          await recordsWrite.signAsOwner(Jws.createSigner(alice));
          const bobSignature =
            recordsWrite.message.authorization.signature.signatures[0];
          recordsWrite.message.authorization.ownerSignature!.signatures[0].signature =
            bobSignature.signature; // invalid `ownerSignature`
          ...
          expect(aliceWriteReply.status.detail).to.contain(
      ...
      describe("should return 400 if actual data size mismatches with `dataSize` in descriptor", () => {
        it("with dataStream and `dataSize` larger than encodedData threshold", async () => {
          ...
          message.descriptor.dataSize =
            DwnConstant.maxDataSizeAllowedToBeEncoded + 100;
          const descriptorCid = await Cid.computeCid(message.descriptor);
          const recordId = await RecordsWrite.getEntryId(
            alice.did,
            message.descriptor
          );
      ...
      it("should return 400 for if dataStream is not present for a write after a delete", async () => {
        ...
        expect(withoutDataReply.status.detail).to.contain(
          DwnErrorCode.RecordsWriteMissingDataStream
        );
        const updatedWriteData = DataStream.fromBytes(dataBytes!);
        const withoutDataReply2 = await dwn.processMessage(
          tenant,
          write.message,
          { dataStream: updatedWriteData }
        );
      ...
      it("should return 400 for data CID mismatch with both dataStream and `dataSize` larger than encodedData threshold", async () => {
        ...
        const dataStream = DataStream.fromBytes(
          TestDataGenerator.randomBytes(
            DwnConstant.maxDataSizeAllowedToBeEncoded + 1
          )
        ); // mismatch data stream
      ...
      it('#359 - should not allow access of data by referencing a different`dataCid` in "modify" `RecordsWrite` with large data', async () => {
        ...
        const dataString = TestDataGenerator.randomString(
          DwnConstant.maxDataSizeAllowedToBeEncoded + 1
        );
        ...
        const write1Reply = await dwn.processMessage(
          alice.did,
          write1.message,
          { dataStream: write1.dataStream }
        );
        ...
        const write2Reply = await dwn.processMessage(
          alice.did,
          write2.message,
          { dataStream: write2.dataStream }
        );
        ...
        const write2ChangeReply = await dwn.processMessage(
          alice.did,
          write2Change.message
        );
        expect(write2ChangeReply.status.code).to.equal(400); // should be disallowed
        expect(write2ChangeReply.status.detail).to.contain(
          DwnErrorCode.RecordsWriteDataCidMismatch
        );
      ...
      describe("initial write & subsequent write tests", () => {
        describe("createFrom()", () => {
          it("should accept a published RecordsWrite using createFrom() without specifying `data` or `datePublished`", async () => {
            ...
            expect(recordsQueryReply.status.code).to.equal(200);
            expect(recordsQueryReply.entries?.length).to.equal(1);
            expect(recordsQueryReply.entries![0].descriptor.published).to.equal(
              true
            );
          ...
          it("should inherit parent published state when using createFrom() to create RecordsWrite", async () => {
            ...
            const newWriteReply = await dwn.processMessage(
              tenant,
              newWrite.message,
              { dataStream: DataStream.fromBytes(newData) }
            );
            ...
            expect(recordsWriteReturned.encodedData).to.equal(
              Encoder.bytesToBase64Url(newData)
            );
            expect(recordsWriteReturned.descriptor.published).to.equal(true);
            expect(recordsWriteReturned.descriptor.datePublished).to.equal(
        ...
        it("should return 400 if `contextId` in an initial protocol-base write mismatches with the expected deterministic `contextId`", async () => {
          ...
          const reply = await dwn.processMessage("unused-tenant-DID", message, {
            dataStream,
          });
          expect(reply.status.code).to.equal(400);
          expect(reply.status.detail).to.contain(
        ...
        describe("event log", () => {
          it("should add an event to the event log on initial write", async () => {
            ...
            const reply = await dwn.processMessage(author.did, message, {
              dataStream,
            });
            expect(reply.status.code).to.equal(202);
      ...
      describe("protocol based writes", () => {
        it("should allow write with allow-anyone rule", async () => {
          ...
          const protocolDefinition =
            emailProtocolDefinition as ProtocolDefinition;
          ...
          const bobWriteReply = await dwn.processMessage(
            alice.did,
            emailFromBob.message,
            { dataStream: emailFromBob.dataStream }
          );
          ...
          const bobRecordsQueryReply = await dwn.processMessage(
            alice.did,
          ...
          expect(bobRecordsQueryReply.entries![0].encodedData).to.equal(
            Encoder.bytesToBase64Url(bobData)
          );
        });
        ...
        it("should allow update with allow-anyone rule", async () => {
          ...
          const docRecordsReply = await dwn.processMessage(
            alice.did,
            docRecord.message,
            { dataStream: docRecord.dataStream }
          );
          ...
          const docUpdateRecord =
            await TestDataGenerator.generateFromRecordsWrite({
              author: bob,
              existingWrite: docRecord.recordsWrite,
              data: bobsData,
            });
          const docUpdateRecordsReply = await dwn.processMessage(
            alice.did,
            docUpdateRecord.message,
            { dataStream: docUpdateRecord.dataStream }
          );
          ...
          const bobDocRecordsReply = await dwn.processMessage(
            alice.did,
            bobDocRecord.message,
            { dataStream: bobDocRecord.dataStream }
          );
          expect(bobDocRecordsReply.status.code).to.equal(401);
          expect(bobDocRecordsReply.status.detail).to.contain(
        ...
        describe("recipient rules", () => {
          it("should allow write with ancestor recipient rule", async () => {
            ...
            const credentialApplicationSchema =
              protocolDefinition.types.credentialApplication.schema;
            const credentialResponseSchema =
              protocolDefinition.types.credentialResponse.schema;
            ...
            const credentialApplicationContextId =
              await credentialApplication.recordsWrite.getEntryId();
            ...
            expect(
              applicationResponseQueryReply.entries![0].encodedData
          ...
          it("should allow update with ancestor recipient rule", async () => {
            ...
            const tagRecordsReply = await dwn.processMessage(
              alice.did,
              tagRecord.message,
              { dataStream: tagRecord.dataStream }
            );
            expect(tagRecordsReply.status.code).to.equal(202);

            // Bob updates Alice's post
            const bobsData = await TestDataGenerator.randomBytes(10);
            const tagUpdateRecord =
              await TestDataGenerator.generateFromRecordsWrite({
                author: bob,
                existingWrite: tagRecord.recordsWrite,
                data: bobsData,
              });
            const tagUpdateRecordsReply = await dwn.processMessage(
              alice.did,
              tagUpdateRecord.message,
              { dataStream: tagUpdateRecord.dataStream }
            );
            ...
            const bobTagRecordsReply = await dwn.processMessage(
              alice.did,
              bobTagRecord.message,
              { dataStream: bobTagRecord.dataStream }
            );
            expect(bobTagRecordsReply.status.code).to.equal(401);
            expect(bobTagRecordsReply.status.detail).to.contain(
              DwnErrorCode.ProtocolAuthorizationActionNotAllowed
            );
          });
          ...
          it("should allowed update with direct recipient rule", async () => {
            ...
            const protocolDefinition =
              recipientCanProtocol as ProtocolDefinition;
            const alice = await TestDataGenerator.generatePersona();
            const bob = await TestDataGenerator.generatePersona();
            const carol = await TestDataGenerator.generatePersona();
            ...
            const recordsWriteReply = await dwn.processMessage(
              alice.did,
              recordsWrite.message,
              { dataStream: recordsWrite.dataStream }
            );
            expect(recordsWriteReply.status.code).to.eq(202);

            // Carol is unable to update the 'post'
            const carolRecordsWrite =
              await TestDataGenerator.generateFromRecordsWrite({
                author: carol,
                existingWrite: recordsWrite.recordsWrite,
              });
            const carolRecordsWriteReply = await dwn.processMessage(
              alice.did,
              carolRecordsWrite.message
            );
            expect(carolRecordsWriteReply.status.code).to.eq(401);
            expect(carolRecordsWriteReply.status.detail).to.contain(
              DwnErrorCode.ProtocolAuthorizationActionNotAllowed
            );
            ...
            const bobRecordsWriteReply = await dwn.processMessage(
              alice.did,
              bobRecordsWrite.message,
              { dataStream: bobRecordsWrite.dataStream }
            );
        ...
        describe("author action rules", () => {
          it("allow author to write with ancestor author rule and block non-authors", async () => {
            ...
            const imageReply = await dwn.processMessage(
              bob.did,
              imageRecordsWrite.message,
              { dataStream: imageRecordsWrite.dataStream }
            );
            expect(imageReply.status.code).to.equal(202);

            const imageContextId =
              await imageRecordsWrite.recordsWrite.getEntryId();
            ...
            const captionReply = await dwn.processMessage(
              bob.did,
              captionImposter.message,
              { dataStream: captionImposter.dataStream }
            );
            expect(captionReply.status.code).to.equal(401);
            expect(captionReply.status.detail).to.contain(
              DwnErrorCode.ProtocolAuthorizationActionNotAllowed
            );
            ...
            const captionResponse = await dwn.processMessage(
              bob.did,
              captionRecordsWrite.message,
              { dataStream: captionRecordsWrite.dataStream }
            );
            ...
            const applicationResponseQueryReply = await dwn.processMessage(
              bob.did,
              messageDataForQueryingCaptionResponse.message
            );
            expect(applicationResponseQueryReply.status.code).to.equal(200);
            expect(applicationResponseQueryReply.entries?.length).to.equal(1);
            expect(
          ...
          it("should allow update with ancestor author rule", async () => {
            ...
            const postRecordsReply = await dwn.processMessage(
              alice.did,
              postRecord.message,
              { dataStream: postRecord.dataStream }
            );
            ...
            const commentRecordsReply = await dwn.processMessage(
              alice.did,
              commentRecord.message,
              { dataStream: commentRecord.dataStream }
            );
            expect(commentRecordsReply.status.code).to.equal(202);

            // Bob updates Alice's comment
            const bobsData = await TestDataGenerator.randomBytes(10);
            const postUpdateRecord =
              await TestDataGenerator.generateFromRecordsWrite({
                author: alice,
                existingWrite: commentRecord.recordsWrite,
                data: bobsData,
              });
            const commentUpdateRecordsReply = await dwn.processMessage(
              alice.did,
              postUpdateRecord.message,
              { dataStream: postUpdateRecord.dataStream }
            );
            ...
            const bobPostRecordsReply = await dwn.processMessage(
              alice.did,
              bobPostRecord.message,
              { dataStream: bobPostRecord.dataStream }
            );
            expect(bobPostRecordsReply.status.code).to.equal(401);
            expect(bobPostRecordsReply.status.detail).to.contain(
              DwnErrorCode.ProtocolAuthorizationActionNotAllowed
            );
          });
        });
        ...
        describe("role rules", () => {
          describe("write $globalRole records", () => {
            it("allows a $globalRole record with unique recipient to be created and updated", async () => {
              ...
              const friendRoleReply = await dwn.processMessage(
                alice.did,
                friendRoleRecord.message,
                { dataStream: friendRoleRecord.dataStream }
              );
              ...
              const updateFriendReply = await dwn.processMessage(
                alice.did,
                updateFriendRecord.message,
                { dataStream: updateFriendRecord.dataStream }
              );
            ...
            it("rejects writes to a $globalRole if recipient is undefined", async () => {
              ...
              expect(friendRoleReply.status.detail).to.contain(
            ...
            it("rejects writes to a $globalRole if there is already a record with the same role and recipient", async () => {
              ...
              const duplicateFriendReply = await dwn.processMessage(
                alice.did,
                duplicateFriendRecord.message,
                { dataStream: duplicateFriendRecord.dataStream }
              );
              expect(duplicateFriendReply.status.code).to.equal(400);
              expect(duplicateFriendReply.status.detail).to.contain(
          ...
          describe("write contextRole records", () => {
            it("allows a $contextRole record with recipient unique to the context to be created and updated", async () => {
              ...
              const threadRecordReply = await dwn.processMessage(
                alice.did,
                threadRecord.message,
                { dataStream: threadRecord.dataStream }
              );
              ...
              const participantRecordReply = await dwn.processMessage(
                alice.did,
                participantRecord.message,
                { dataStream: participantRecord.dataStream }
              );
              expect(participantRecordReply.status.code).to.equal(202);

              // Alice updates Bob's role record
              const participantUpdateRecord =
                await TestDataGenerator.generateFromRecordsWrite({
                  author: alice,
                  existingWrite: participantRecord.recordsWrite,
                });
              const participantUpdateRecordReply = await dwn.processMessage(
                alice.did,
                participantUpdateRecord.message,
                { dataStream: participantUpdateRecord.dataStream }
              );
            ...
            it("allows a $contextRole record to be created even if there is a $contextRole in a different context", async () => {
              ...
              const threadRecordReply1 = await dwn.processMessage(
                alice.did,
                threadRecord1.message,
                { dataStream: threadRecord1.dataStream }
              );
              expect(threadRecordReply1.status.code).to.equal(202);

              // Alice adds Bob as a 'thread/participant' to the first thread
              const participantRecord1 =
                await TestDataGenerator.generateRecordsWrite({
                  author: alice,
                  recipient: bob.did,
                  protocol: protocolDefinition.protocol,
              ...
              const participantRecordReply1 = await dwn.processMessage(
                alice.did,
                participantRecord1.message,
                { dataStream: participantRecord1.dataStream }
              );
              ...
              const threadRecordReply2 = await dwn.processMessage(
                alice.did,
                threadRecord2.message,
                { dataStream: threadRecord2.dataStream }
              );
              expect(threadRecordReply2.status.code).to.equal(202);

              // Alice adds Bob as a 'thread/participant' to the second thread
              const participantRecord2 =
                await TestDataGenerator.generateRecordsWrite({
                  author: alice,
                  recipient: bob.did,
                  protocol: protocolDefinition.protocol,
              ...
              const participantRecordReply2 = await dwn.processMessage(
                alice.did,
                participantRecord2.message,
                { dataStream: participantRecord2.dataStream }
              );
            ...
            it("rejects writes to a $contextRole record if there already exists one in the same context", async () => {
              ...
              expect(participantRecordReply2.status.detail).to.contain(
            ...
            it("allows a new $contextRole record to be created for the same recipient in the same context if their old one was deleted", async () => {
              ...
              const participantDelete =
                await TestDataGenerator.generateRecordsDelete({
                  author: alice,
                  recordId: participantRecord1.message.recordId,
                });
              const participantDeleteReply = await dwn.processMessage(
                alice.did,
                participantDelete.message
              );
          ...
          describe("protocolRole based writes", () => {
            it("uses a globalRole to authorize a write", async () => {
              ...
              const chatRecord = await TestDataGenerator.generateRecordsWrite({
                ...
                data: new TextEncoder().encode(
                  "Bob can write this cuz he is Alices friend"
                ),
                protocolRole: "friend",
              });
              const chatReply = await dwn.processMessage(
                alice.did,
                chatRecord.message,
                { dataStream: chatRecord.dataStream }
              );
            ...
            it("uses a $globalRole to authorize an update", async () => {
              ...
              const chatUpdateRecord =
                await TestDataGenerator.generateFromRecordsWrite({
                  author: bob,
                  existingWrite: chatRecord.recordsWrite,
                  protocolRole: "admin",
                });
              const chatUpdateReply = await dwn.processMessage(
                alice.did,
                chatUpdateRecord.message,
                { dataStream: chatUpdateRecord.dataStream }
              );
            ...
            it("rejects role-authorized writes if the protocolRole is not a valid protocol path to a role record", async () => {
              ...
              const chatReadReply = await dwn.processMessage(
                alice.did,
                writeChatRecord.message,
                { dataStream: writeChatRecord.dataStream }
              );
              expect(chatReadReply.status.code).to.equal(401);
              expect(chatReadReply.status.detail).to.contain(
...
=======
              expect(chatReply.status.detail).to.contain(
...
>>>>>>> 396c98214e54607dd6087a0deccee3b98d830a09
            ...
            it("uses a contextRole to authorize a write", async () => {
              ...
              const chatRecordReply = await dwn.processMessage(
                alice.did,
                chatRecord.message,
                { dataStream: chatRecord.dataStream }
              );
            ...
            it("uses a contextRole to authorize an update", async () => {
              ...
              const chatUpdateRecordReply = await dwn.processMessage(
                alice.did,
                chatUpdateRecord.message,
                { dataStream: chatUpdateRecord.dataStream }
              );
...
=======
              expect(chatRecordReply.status.detail).to.contain(
                DwnErrorCode.ProtocolAuthorizationMatchingRoleRecordNotFound
              );
>>>>>>> 396c98214e54607dd6087a0deccee3b98d830a09
            });
            ...
            it("rejects attempts to invoke an invalid path as a protocolRole", async () => {
              ...
              const fakeRoleInvocationReply = await dwn.processMessage(
                alice.did,
                fakeRoleInvocation.message,
                { dataStream: fakeRoleInvocation.dataStream }
              );
              expect(fakeRoleInvocationReply.status.code).to.equal(401);
              expect(fakeRoleInvocationReply.status.detail).to.contain(
                DwnErrorCode.ProtocolAuthorizationNotARole
              );
            });
          });
        ...
        it("should allow updating records by the initial author", async () => {
          ...
          const bobWriteReply = await dwn.processMessage(
            ...
            { dataStream: messageFromBob.dataStream }
          );
          expect(bobWriteReply.status.code).to.equal(202);
          ...
          const newWriteReply = await dwn.processMessage(
            alice.did,
            updatedMessageFromBob.message,
            { dataStream: updatedMessageFromBob.dataStream }
          );
          expect(newWriteReply.status.code).to.equal(202);

          // verify bob's message got written to the DB
          const newRecordQueryReply = await dwn.processMessage(
            alice.did,
            messageDataForQueryingBobsWrite.message
          );
          expect(newRecordQueryReply.status.code).to.equal(200);
          expect(newRecordQueryReply.entries?.length).to.equal(1);
          expect(newRecordQueryReply.entries![0].encodedData).to.equal(
            Encoder.bytesToBase64Url(updatedMessageBytes)
          );
        });
        ...
        it("should disallow overwriting existing records by a different author if author is not authorized to `update`", async () => {
          ...
          const carolWriteReply = await dwn.processMessage(
            alice.did,
            modifiedMessageFromCarol.message,
            { dataStream: modifiedMessageFromCarol.dataStream }
          );
          expect(carolWriteReply.status.code).to.equal(401);
          expect(carolWriteReply.status.detail).to.contain(
            DwnErrorCode.ProtocolAuthorizationActionNotAllowed
        ...
        it("should block unauthorized write with recipient rule", async () => {
          ...
          expect(credentialResponseReply.status.detail).to.contain(
            DwnErrorCode.ProtocolAuthorizationActionNotAllowed
          );
        });
        ...
        it("should fail authorization if given `dataFormat` is mismatching with the dataFormats in protocol definition", async () => {
          ...
          const replyMatch = await dwn.processMessage(
            alice.did,
            recordsWriteMatch.message,
            { dataStream: recordsWriteMatch.dataStream }
          );
          ...
          const replyMismatch = await dwn.processMessage(
            alice.did,
            recordsWriteMismatch.message,
            { dataStream: recordsWriteMismatch.dataStream }
          );
          expect(replyMismatch.status.code).to.equal(400);
          expect(replyMismatch.status.detail).to.contain(
        ...
        it("should fail authorization if record schema is not allowed at the hierarchical level attempted for the RecordsWrite", async () => {
          ...
          const failedCredentialResponseReply = await dwn.processMessage(
            alice.did,
            failedCredentialResponse.message,
            { dataStream: failedCredentialResponse.dataStream }
          );
          expect(failedCredentialResponseReply.status.code).to.equal(400);
          expect(failedCredentialResponseReply.status.detail).to.contain(
          ...
          const failedCredentialApplicationReply2 = await dwn.processMessage(
            alice.did,
            failedCredentialApplication.message,
            { dataStream: failedCredentialApplication.dataStream }
          );
          expect(failedCredentialApplicationReply2.status.code).to.equal(400);
          expect(failedCredentialApplicationReply2.status.detail).to.contain(
            DwnErrorCode.ProtocolAuthorizationMissingRuleSet
          );
          ...
          const nestedCredentialApplicationReply = await dwn.processMessage(
            alice.did,
            nestedCredentialApplication.message,
            { dataStream: nestedCredentialApplication.dataStream }
          );
          expect(nestedCredentialApplicationReply.status.code).to.equal(400);
          expect(nestedCredentialApplicationReply.status.detail).to.contain(
            DwnErrorCode.ProtocolAuthorizationMissingRuleSet
          );
        });
        ...
        it("should only allow DWN owner to write if record does not have an action rule defined", async () => {
          ...
          let reply = await dwn.processMessage(
            alice.did,
            aliceWriteMessageData.message,
            { dataStream: aliceWriteMessageData.dataStream }
          );
          expect(reply.status.code).to.equal(202);
          ...
          reply = await dwn.processMessage(
            alice.did,
            bobWriteMessageData.message,
            { dataStream: bobWriteMessageData.dataStream }
          );
        ...
        it("should look up recipient path with ancestor depth of 2+ (excluding self) in action rule correctly", async () => {
          ...
          let reply = await dwn.processMessage(
            pfi.did,
            askMessageData.message,
            { dataStream: askMessageData.dataStream }
          );
          expect(reply.status.code).to.equal(202);
          ...
          reply = await dwn.processMessage(pfi.did, offerMessageData.message, {
            dataStream: offerMessageData.dataStream,
          });
          expect(reply.status.code).to.equal(202);
          ...
          reply = await dwn.processMessage(
            pfi.did,
            fulfillmentMessageData.message,
            { dataStream: fulfillmentMessageData.dataStream }
          );
          expect(reply.status.code).to.equal(202);
        ...
        it("should 400 if expected CID of `encryption` mismatches the `encryptionCid` in `authorization`", async () => {
          ...
          const bobMessageEncryptedBytes = await DataStream.toBytes(
            bobMessageEncryptedStream
          );
        ...
        it("should return 400 if protocol is not normalized", async () => {
          ...
          const attestation = await RecordsWrite.createAttestation(
            descriptorCid
          );
          const signature = await RecordsWrite.createSignerSignature({
          ...
          expect(reply.status.code).to.equal(400);
          expect(reply.status.detail).to.contain(
            DwnErrorCode.UrlProtocolNotNormalized
          );
        });
        ...
        it("#359 - should not allow access of data by referencing `dataCid` in protocol authorized `RecordsWrite`", async () => {
          ...
          const bobRecordsReadReply = await dwn.processMessage(
            alice.did,
            bobRecordsReadData.message
          );
          ...
          const bobRecordsQueryRecordIdReply = await dwn.processMessage(
            alice.did,
            bobRecordsQueryReordId.message
          );
          ...
          const updateRecordReply = await dwn.processMessage(
            alice.did,
            updateRecord.message
          );
          expect(updateRecordReply.status.code).to.equal(400);
          expect(updateRecordReply.status.detail).to.include(
            DwnErrorCode.RecordsWriteMissingEncodedDataInPrevious
          );

          // sanity still can't query
          const bobRecordsQueryReply2 = await dwn.processMessage(
            alice.did,
            bobRecordsQuery.message
          );
        ...
        it("should allow record with or without schema if protocol does not require schema for a record type", async () => {
          ...
          const protocolDefinition =
            anyoneCollaborateProtocolDefinition as ProtocolDefinition;
          const alice = await TestDataGenerator.generateDidKeyPersona();
          ...
          const recordsReadReply = await dwn.processMessage(
            alice.did,
...
=======
          ...
          const reply = await dwn.processMessage(
            alice.did,
            testRecord.message,
            { dataStream: testRecord.dataStream }
          );
>>>>>>> 396c98214e54607dd6087a0deccee3b98d830a09
          expect(reply.status.code).to.equal(202);
...
=======
          ...
          const reply2 = await dwn.processMessage(
            alice.did,
            testRecord2.message,
            { dataStream: testRecord2.dataStream }
          );
>>>>>>> 396c98214e54607dd6087a0deccee3b98d830a09
...
=======
          ...
          const reply3 = await dwn.processMessage(
            alice.did,
            testRecord3.message,
            { dataStream: testRecord3.dataStream }
          );
          expect(reply3.status.code).to.equal(400);
          expect(reply3.status.detail).to.contain(
...
>>>>>>> 396c98214e54607dd6087a0deccee3b98d830a09
          ...
          const foo1WriteResponse = await dwn.processMessage(
            alice.did,
            foo1.message,
            { dataStream: foo1.dataStream }
          );
          ...
          const deleteFooReply = await dwn.processMessage(
            alice.did,
            deleteFoo.message
          );
          ...
          const bar1WriteResponse = await dwn.processMessage(
            alice.did,
            bar1.message,
            { dataStream: bar1.dataStream }
          );
          expect(bar1WriteResponse.status.code).equals(400);
          expect(bar1WriteResponse.status.detail).to.contain(
            DwnErrorCode.ProtocolAuthorizationIncorrectProtocolPath
          );
        });
      ...
      describe("grant based writes", () => {
        it("allows external parties to write a record using a grant with unrestricted RecordsWrite scope", async () => {
          ...
          const permissionsGrantId: string = await Message.getCid(
            permissionsGrant.message
          );
          ...
          const recordsWriteReply = await dwn.processMessage(
            alice.did,
            protocolRecordsWrite.message,
            { dataStream: protocolRecordsWrite.dataStream }
          );
          expect(recordsWriteReply.status.code).to.equal(202);
          ...
          const recordsWriteReply2 = await dwn.processMessage(
            alice.did,
            nonProtocolRecordsWrite.message,
            { dataStream: nonProtocolRecordsWrite.dataStream }
          );
        ...
        describe("protocol records", () => {
          ...
          it("rejects writes of protocol records with mismatching protocol grant scopes", async () => {
            ...
            expect(recordsWriteReply.status.code).to.equal(401);
            expect(recordsWriteReply.status.detail).to.contain(
          ...
          it("allows writes of protocol records with matching contextId grant scopes", async () => {
            ...
            const alicesRecordsWriteReply = await dwn.processMessage(
              alice.did,
              alicesRecordsWrite.message,
              { dataStream: alicesRecordsWrite.dataStream }
            );
            ...
            const bobsRecordsWriteReply = await dwn.processMessage(
              alice.did,
              bobsRecordsWrite.message,
              { dataStream: bobsRecordsWrite.dataStream }
            );
          ...
          it("rejects writes of protocol records with mismatching contextId grant scopes", async () => {
            ...
            expect(bobsRecordsWriteReply.status.detail).to.contain(
        ...
        describe("grant condition published", () => {
          it("Rejects unpublished records if grant condition `published` === required", async () => {
            ...
            const permissionsGrantId = await Message.getCid(
              permissionsGrant.message
            );
            ...
            const unpublishedRecordsWriteReply = await dwn.processMessage(
              alice.did,
              unpublishedRecordsWrite.message,
              { dataStream: unpublishedRecordsWrite.dataStream }
            );
            expect(unpublishedRecordsWriteReply.status.code).to.equal(401);
            expect(unpublishedRecordsWriteReply.status.detail).to.contain(
          ...
          it("Rejects published records if grant condition `published` === prohibited", async () => {
            ...
            expect(publishedRecordsWriteReply.status.detail).to.contain(
      ...
      it("should 400 if dataStream is not provided and dataStore does not contain dataCid", async () => {
        ...
        const prunedRecordsWriteReply = await dwn.processMessage(
          alice.did,
          prunedRecordsWrite.message
        );
      ...
      describe("reference counting tests", () => {
        it("should not allow referencing data across tenants", async () => {
          ...
          const aliceWriteReply = await dwn.processMessage(
            alice.did,
            aliceWriteData.message,
            { dataStream: aliceWriteData.dataStream }
          );
          expect(aliceWriteReply.status.code).to.equal(202);
          ...
          const aliceQueryWriteAfterAliceWriteReply = await dwn.processMessage(
            alice.did,
            aliceQueryWriteAfterAliceWriteData.message
          );
          expect(aliceQueryWriteAfterAliceWriteReply.status.code).to.equal(200);
          expect(aliceQueryWriteAfterAliceWriteReply.entries?.length).to.equal(
            1
          );
          expect(
            aliceQueryWriteAfterAliceWriteReply.entries![0].encodedData
          ).to.equal(encodedData);
          ...
          const bobAssociateReply = await dwn.processMessage(
            bob.did,
            bobAssociateData.message,
            { dataStream: bobAssociateData.dataStream }
          );
          ...
          const aliceQueryWriteAfterBobAssociateReply =
            await dwn.processMessage(
              alice.did,
              aliceQueryWriteAfterBobAssociateData.message
            );
          expect(aliceQueryWriteAfterBobAssociateReply.status.code).to.equal(
            200
          );
          expect(
            aliceQueryWriteAfterBobAssociateReply.entries?.length
          ).to.equal(1);
          expect(
            aliceQueryWriteAfterBobAssociateReply.entries![0].encodedData
          ).to.equal(encodedData);
          ...
          const bobQueryAssociateAfterBobAssociateReply =
            await dwn.processMessage(
              bob.did,
              bobQueryAssociateAfterBobAssociateData.message
            );
          expect(bobQueryAssociateAfterBobAssociateReply.status.code).to.equal(
            200
          );
          expect(
      ...
      describe("encodedData threshold", async () => {
        it("should call cloneAndAddEncodedData and not validateDataStoreIntegrity if dataSize is less than or equal to the threshold", async () => {
          ...
          const processEncoded = sinon.spy(
            RecordsWriteHandler.prototype as any,
            "cloneAndAddEncodedData"
          );
          const validateStore = sinon.spy(
            RecordsWriteHandler.prototype as any,
            "validateDataStoreIntegrity"
          );

          const writeMessage = await dwn.processMessage(alice.did, message, {
            dataStream,
          });
        ...
        it("should have encodedData field if dataSize is less than or equal to the threshold", async () => {
          ...
          expect((storedMessage as RecordsQueryReplyEntry).encodedData).to.exist
        ...
        it("should not have encodedData field if dataSize greater than threshold", async () => {
          ...
          const storedMessage = await messageStore.get(alice.did, messageCid);
          expect((storedMessage as RecordsQueryReplyEntry).encodedData).to.not
            .exist;
        });
        ...
        it("should retain original RecordsWrite message but without the encodedData if data is under threshold", async () => {
          ...
          const updatedDataBytes = TestDataGenerator.randomBytes(
            DwnConstant.maxDataSizeAllowedToBeEncoded
          );
          const newWrite = await RecordsWrite.createFrom({
            recordsWriteMessage: message,
          ...
          const writeMessage2 = await dwn.processMessage(
            alice.did,
            newWrite.message,
          ...
          expect((originalWrite as RecordsQueryReplyEntry).encodedData).to.not
            .exist;

          const newestWrite = await messageStore.get(
            alice.did,
            await Message.getCid(newWrite.message)
          );
          expect((newestWrite as RecordsQueryReplyEntry).encodedData).to.exist
            .and.not.be.undefined;
        });
      });
    });
    ...
    describe("authorization validation tests", () => {
      it("should return 400 if `recordId` in payload of the message signature mismatches with `recordId` in the message", async () => {
        ...
        signaturePayload.recordId =
          await TestDataGenerator.randomCborSha256Cid(); // make recordId mismatch in authorization payload
      ...
      it("should return 400 if `contextId` in payload of message signature mismatches with `contextId` in the message", async () => {
        ...
        const signaturePayload = { ...recordsWrite.signaturePayload };
        signaturePayload.contextId =
          await TestDataGenerator.randomCborSha256Cid(); // make contextId mismatch in authorization payload
        const signaturePayloadBytes = Encoder.objectToBytes(signaturePayload);
        const signer = Jws.createSigner(author);
        const jwsBuilder = await GeneralJwsBuilder.create(
          signaturePayloadBytes,
    ...
    describe("attestation validation tests", () => {
      it("should fail with 400 if `attestation` payload contains properties other than `descriptorCid`", async () => {
        ...
        const attestationBuilder = await GeneralJwsBuilder.create(
        ...
        const signaturePayload = { ...recordsWrite.signaturePayload };
        signaturePayload.attestationCid = await Cid.computeCid(
          attestationPayload
        );
        const signaturePayloadBytes = Encoder.objectToBytes(signaturePayload);
        const authorizationBuilder = await GeneralJwsBuilder.create(
          signaturePayloadBytes,
          [signer]
        );
        ...
        expect(reply.status.code).to.equal(400);
        expect(reply.status.detail).to.contain(
          `Only 'descriptorCid' is allowed in attestation payload`
        );
      });
      ...
      it("should fail validation with 400 if the `attestation` does not include the correct `descriptorCid`", async () => {
        ...
        const anotherWrite = await TestDataGenerator.generateRecordsWrite({
          attesters: [alice],
        });
    ...
    describe("unknown error", () => {
      ...
      it("should throw if `recordsWriteHandler.processMessageWithoutDataStream()` throws unknown error", async () => {
        ...
        sinon
          .stub(recordsWriteHandler as any, "processMessageWithoutDataStream")
          .throws(
            new Error(
        ...
        sinon
          .stub(recordsWriteHandler as any, "processMessageWithDataStream")
          .throws(
            new Error(
        ...
        let handlerPromise = recordsWriteHandler.handle({
          tenant,
          message,
          dataStream: dataStream!,
        }); // with data stream
