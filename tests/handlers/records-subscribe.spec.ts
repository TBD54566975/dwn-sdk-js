  ...
  describe('RecordsSubscribeHandler.handle()', () => {
    describe('functional tests', () => {
      ...
      it('should return 401 for anonymous subscriptions that filter explicitly for unpublished records', async () => {
        ...
        const unpublishedPostReply = await dwn.processMessage(alice.did, unpublishedPostSubscribe.message);
      ...
      it('should return 401 if signature check fails', async () => {
        ...
        const reply = await recordsSubscribeHandler.handle({ tenant, message });
      ...
      describe('protocol based subscriptions', () => {
        it('does not try protocol authorization if protocolRole is not invoked', async () => {
          ...
          const subscriptionReply = await dwn.processMessage(alice.did, bobSubscription.message);
          ...
          subscriptionReply.subscription?.on(addCid);
        ...
        it('allows $globalRole authorized subscriptions', async () => {
          ...
          const subscriptionReply = await dwn.processMessage(alice.did, noRoleSubscription.message);
          expect(subscriptionReply.status.code).to.equal(200);
          expect(subscriptionReply.subscription).to.exist;
          ...
          subscriptionReply.subscription?.on(addNoRole);
          ...
          const subscriptionWithRoleReply = await dwn.processMessage(alice.did, bobSubscriptionWithRole.message);
          ...
          subscriptionWithRoleReply.subscription?.on(addRecord);
