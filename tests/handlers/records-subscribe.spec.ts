...
import type { RecordsDeleteMessage, RecordsFilter } from '../../src/types/records-types.js';
  ...
  describe('RecordsSubscribeHandler.handle()', () => {
    describe('functional tests', () => {
      ...
      it('should return a subscription object', async () => {
        ...
        const reply = await dwn.processMessage(alice.did, recordsSubscribe.message, { handler: () => {} });
      ...
      it('should return 401 for anonymous subscriptions that filter explicitly for unpublished records', async () => {
        ...
        const unpublishedPostReply = await dwn.processMessage(alice.did, unpublishedPostSubscribe.message, { handler: () => {} });
      ...
      it('should return 401 if signature check fails', async () => {
        ...
        const reply = await recordsSubscribeHandler.handle({ tenant, message, handler: () => {} });
      ...
      describe('protocol based subscriptions', () => {
        it('does not try protocol authorization if protocolRole is not invoked', async () => {
          ...
          const subscriptionReply = await dwn.processMessage(alice.did, bobSubscription.message, { handler: addCid });
        ...
        it('allows $globalRole authorized subscriptions', async () => {
          ...
          const subscriptionReply = await dwn.processMessage(alice.did, noRoleSubscription.message, { handler: addNoRole });
          expect(subscriptionReply.status.code).to.equal(200);
          expect(subscriptionReply.subscription).to.exist;
          ...
          const addRecord = async (message: GenericMessage): Promise<void> => {
            if (message.descriptor.interface === DwnInterfaceName.Records && message.descriptor.method === DwnMethodName.Write) {
              const recordsWriteMessage = message as RecordsWriteMessage;
          ...
          const subscriptionWithRoleReply = await dwn.processMessage(alice.did, bobSubscriptionWithRole.message, { handler: addRecord });
        ...
        it('allows protocol authorized subscriptions', async () => {
          // scenario: Alice writes some chat messages writes a chat message.
        ...
        it('does not execute $contextRole authorized subscriptions where contextId is missing from the filter', async () => {
          ...
          //           does not contain a contextId so the subscription fails.
          const alice = await DidKeyResolver.generate();
          const bob = await DidKeyResolver.generate();
          ...
          // Bob invokes his thread participant role to subscribe
          const chatSubscribe = await TestDataGenerator.generateRecordsSubscribe({
            author : bob,
            filter : {
              protocol     : protocolDefinition.protocol,
        ...
        it('rejects $globalRole authorized subscriptions if the request author does not have a matching $globalRole', async () => {
          ...
          // Bob invokes his friendRole to subscribe to the records
          const chatSubscribe = await TestDataGenerator.generateRecordsSubscribe({
            author : bob,
            filter : {
              protocol     : protocolDefinition.protocol,
