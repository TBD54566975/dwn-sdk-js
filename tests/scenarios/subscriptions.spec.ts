...
export function testSubscriptionScenarios(): void {
  describe('subscriptions', () => {
    describe('without reauthorization', () => {
      ...
      describe('records subscribe', () => {
        it('filters by protocol', async () => {
          ...
          await proto1SubscriptionReply.subscription?.close();
        ...
        it('unsubscribes', async () => {
          ...
          const schema1SubscriptionReply = await dwn.processMessage(alice.did, schema1Subscription.message, { handler: schema1Handler });
    ...
    describe('reauthorization', () => {
      ...
      xit('does not reauthorize if TTL is set to zero', async () => {
        const eventStream = new EventStreamEmitter();
        dwn = await Dwn.create({ didResolver, messageStore, dataStore, eventLog, eventStream });
        ...
        const bobFriendReply = await dwn.processMessage(alice.did, bobFriend.message, { dataStream: bobFriend.dataStream });
        ...
        const bobSubscribeReply = await dwn.processMessage(alice.did, bobSubscribe.message, { handler: captureFunction });
        ...
        const aliceMessage1Reply = await dwn.processMessage(alice.did, aliceMessage1.message, { dataStream: aliceMessage1.dataStream });
        ...
        const aliceMessage2Reply = await dwn.processMessage(alice.did, aliceMessage2.message, { dataStream: aliceMessage2.dataStream });
      ...
      xit('reauthorize on every event emitted if TTL is less than zero', async () => {
        const eventStream = new EventStreamEmitter();
        dwn = await Dwn.create({ didResolver, messageStore, dataStore, eventLog, eventStream });
      ...
      xit('reauthorizes after the ttl', async () => {
        const clock = sinon.useFakeTimers();

        const eventStream = new EventStreamEmitter(); // every second
        dwn = await Dwn.create({ didResolver, messageStore, dataStore, eventLog, eventStream });
        ...
        const aliceMessage3Reply = await dwn.processMessage(alice.did, aliceMessage3.message, { dataStream: aliceMessage3.dataStream });
      ...
      xit('no longer sends to subscription handler if subscription becomes un-authorized', async () => {
        const eventStream = new EventStreamEmitter();
        dwn = await Dwn.create({ didResolver, messageStore, dataStore, eventLog, eventStream });
