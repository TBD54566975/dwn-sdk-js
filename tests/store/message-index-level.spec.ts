...
import { MessageIndexLevel } from '../../src/store/message-index-leve.js';
...
describe('MessageIndexLevel', () => {
  describe('put', () => {
    let index: MessageIndexLevel;
    let testPartition: LevelWrapper<string>;
    const tenant = 'did:alice:index';
    before(async () => {
      index = new MessageIndexLevel({ location: 'TEST-INDEX' });
      await index.open();
  ...
  describe('encodeNumberValue', () => {
    it('should encode positive digits and pad with leading zeros', () => {
      ...
      const encoded = MessageIndexLevel.encodeNumberValue(100);
    ...
    it('should encode digits to sort using lexicographical comparison', () => {
      ...
      const encodedDigits = digits.map(d => MessageIndexLevel.encodeNumberValue(d))
        .sort((a,b) => lexicographicalCompare(a, b));

      digits.forEach((n,i) => expect(encodedDigits.at(i)).to.equal(MessageIndexLevel.encodeNumberValue(n)));
    });
  });
});