...
describe('IndexLevel', () => {
  ...
  describe('query sortedIndex', () => {
    ...
    it('should not match values prefixed with the query', async () => {
      ...
      const resp = await testIndex.sortedIndexQuery(tenant, [{
        value: 'foo'
      }], { sortProperty: 'id' });
    ...
    describe('booleans', () => {
      it('should return records that match provided boolean equality filter', async () => {
        ...
        // control
        const resp = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'id' });
        expect(resp.length).to.equal(2);
        expect(resp).to.have.members([ itemTrueId, itemFalseId ]);

        // equality true
        const respTrue = await testIndex.sortedIndexQuery(tenant, [{ published: true, schema: 'schema' }], { sortProperty: 'id' });
        expect(respTrue.length).to.equal(1);
        expect(respTrue).to.have.members([ itemTrueId ]);

        // equality false
        const respFalse = await testIndex.sortedIndexQuery(tenant, [{ published: false, schema: 'schema' }], { sortProperty: 'id' });
    ...
    describe('sort, limit and cursor', () => {
      it('only returns the number of results specified by the limit property', async () => {
        ...
        let ascResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', limit: 2 });
        expect(ascResults.length).to.equal(2);
        expect(ascResults).to.eql(['a', 'b']);

        // limit results with a cursor
        ascResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', limit: 2, cursor: 'b' });
        expect(ascResults.length).to.equal(2);
      ...
      it('invalid sort property returns no results', async () => {
        ...
        let validResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val' });
        expect(validResults.length).to.equal(4);

        // sort by invalid property returns no results
        let invalidResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'invalid' });
        expect(invalidResults.length).to.equal(0);

        // control test: returns after cursor
        validResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: 'a' });
        expect(validResults.length).to.equal(3);

        // invalid sort property with a valid cursor value
        invalidResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'invalid', cursor: 'a' });
        expect(invalidResults.length).to.equal(0);
      });
      ...
      it('invalid cursor returns no results', async () => {
        ...
        // verify valid cursor
        const validResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: 'a' });
        expect(validResults.length).to.equal(3);
        expect(validResults).to.eql(['b', 'c', 'd']);

        // invalid cursor
        const invalidResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: 'invalid' });
        expect(invalidResults.length).to.equal(0);
      });
      ...
      it('valid cursor must be within the filtered results', async () => {
        ...
        const validResults = await testIndex.sortedIndexQuery(tenant, [{ val: { gt: 'b' } }], { sortProperty: 'val', cursor: 'c' });
        expect(validResults.length).to.equal(2);
        expect(validResults).to.eql(['d', 'e']);

        // cursor outside of range
        const invalidResults = await testIndex.sortedIndexQuery(tenant, [{ val: { gt: 'b' } }], { sortProperty: 'val', cursor: 'a' });
        expect(invalidResults.length).to.equal(0);
      });
      ...
      it('can sort by any indexed property', async () => {
        ...
        const ascResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val' });
        expect(ascResults.length).to.equal(testVals.length);
        expect(ascResults).to.eql(['a', 'b', 'c', 'd']);

        // sort by index ascending
        const ascIndexResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'index' });
        expect(ascIndexResults.length).to.equal(testVals.length);
        expect(ascIndexResults).eql(testVals);

        // sort by value descending
        const descResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', sortDirection: SortDirection.Descending });
        expect(descResults.length).to.equal(testVals.length);
        expect(descResults).to.eql(['d', 'c', 'b', 'a']);

        // sort by index descending
        const descIndexResults = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'index', sortDirection: SortDirection.Descending });
      ...
      it('sorts lexicographic with and without a cursor', async () => {
        ...
        const ascResultsCursor = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: 'b' });
        ...
        const descResultsCursor = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', sortDirection: SortDirection.Descending, cursor: 'b' });
      ...
      it('sorts numeric with and without a cursor', async () => {
        ...
        const ascResultsCursor = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: '2' });
        expect(ascResultsCursor.length).to.equal(2);
        ...
        const descResultsCursor = await testIndex.sortedIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', sortDirection: SortDirection.Descending, cursor: '2' });
      ...
      it('sorts range queries with or without a cursor', async () => {
        ...
        let response = await testIndex.sortedIndexQuery(tenant, [{
          letter: {
            gte : lowerBound,
            lte : upperBound
          },
        ...
        response = await testIndex.sortedIndexQuery(tenant, [{
          letter: {
            gte : lowerBound,
            lte : upperBound
          },
      ...
      it('sorts range queries negative integers with or without a cursor', async () => {
        ...
        const upperBound = 3;
        const lowerBound = -2;
        let results = await testIndex.sortedIndexQuery(tenant, [{
          digit: {
            gte : lowerBound,
            lte : upperBound
          }
        ...
        results = await testIndex.sortedIndexQuery(tenant, [{
          digit: {
            gte : lowerBound,
            lte : upperBound
          }
      ...
      it('supports multiple filtered queries', async () => {
        ...
        const results = await testIndex.sortedIndexQuery(tenant,[
        ...
        const resultsWithCursor = await testIndex.sortedIndexQuery(tenant, [
          { digit: { gte: lowerBounds, lte: upperBounds } },
          { property: true },
        ], { sortProperty: 'val', cursor: cursor });
        expect(resultsWithCursor).to.eql([...compareResultsAfterCursor], 'results after cursor ascending');

        const descResults = await testIndex.sortedIndexQuery(tenant, [
          { digit: { gte: lowerBounds, lte: upperBounds } },
          { property: true }
        ...
        const descResultsAfterCursor = await testIndex.sortedIndexQuery(tenant, [
          { digit: { gte: lowerBounds, lte: upperBounds } },
          { property: true }
        ], { sortProperty: 'val', sortDirection: SortDirection.Descending, cursor: cursor });
  ...
  describe('query filteredIndex', () => {
    ...
    it('should not match values prefixed with the query', async () => {
      ...
      const resp = await testIndex.filteredIndexQuery(tenant, [{
        value: 'foo'
      }], { sortProperty: 'id' });
    ...
    describe('booleans', () => {
      it('should return records that match provided boolean equality filter', async () => {
        ...
        // control
        const resp = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'id' });
        expect(resp.length).to.equal(2);
        expect(resp).to.have.members([ itemTrueId, itemFalseId ]);

        // equality true
        const respTrue = await testIndex.filteredIndexQuery(tenant, [{ published: true, schema: 'schema' }], { sortProperty: 'id' });
        expect(respTrue.length).to.equal(1);
        expect(respTrue).to.have.members([ itemTrueId ]);

        // equality false
        const respFalse = await testIndex.filteredIndexQuery(tenant, [{ published: false, schema: 'schema' }], { sortProperty: 'id' });
        expect(respFalse.length).to.equal(1);
        expect(respFalse).to.have.members([ itemFalseId ]);
      });
    });
    ...
    describe('sort, limit and cursor', () => {
      it('only returns the number of results specified by the limit property', async () => {
        ...
        // limit results without cursor
        let ascResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', limit: 2 });
        expect(ascResults.length).to.equal(2);
        expect(ascResults).to.eql(['a', 'b']);

        // limit results with a cursor
        ascResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', limit: 2, cursor: 'b' });
        expect(ascResults.length).to.equal(2);
        expect(ascResults).to.eql(['c', 'd']);
      });
      ...
      it('invalid sort property returns no results', async () => {
        ...
        // control test: return all results
        let validResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val' });
        expect(validResults.length).to.equal(4);

        // sort by invalid property returns no results
        let invalidResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'invalid' });
        expect(invalidResults.length).to.equal(0);

        // control test: returns after cursor
        validResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: 'a' });
        expect(validResults.length).to.equal(3);

        // invalid sort property with a valid cursor value
        invalidResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'invalid', cursor: 'a' });
        expect(invalidResults.length).to.equal(0);
      });
      ...
      it('invalid cursor returns no results', async () => {
        ...
        // verify valid cursor
        const validResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: 'a' });
        expect(validResults.length).to.equal(3);
        expect(validResults).to.eql(['b', 'c', 'd']);

        // invalid cursor
        const invalidResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: 'invalid' });
        expect(invalidResults.length).to.equal(0);
      });
      ...
      it('valid cursor must be within the filtered results', async () => {
        ...
        // verify valid cursor within range
        const validResults = await testIndex.filteredIndexQuery(tenant, [{ val: { gt: 'b' } }], { sortProperty: 'val', cursor: 'c' });
        expect(validResults.length).to.equal(2);
        expect(validResults).to.eql(['d', 'e']);

        // cursor outside of range
        const invalidResults = await testIndex.filteredIndexQuery(tenant, [{ val: { gt: 'b' } }], { sortProperty: 'val', cursor: 'a' });
        expect(invalidResults.length).to.equal(0);
      });
      ...
      it('can sort by any indexed property', async () => {
        ...
        // sort by value ascending
        const ascResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val' });
        expect(ascResults.length).to.equal(testVals.length);
        expect(ascResults).to.eql(['a', 'b', 'c', 'd']);

        // sort by index ascending
        const ascIndexResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'index' });
        expect(ascIndexResults.length).to.equal(testVals.length);
        expect(ascIndexResults).eql(testVals);

        // sort by value descending
        const descResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', sortDirection: SortDirection.Descending });
        expect(descResults.length).to.equal(testVals.length);
        expect(descResults).to.eql(['d', 'c', 'b', 'a']);

        // sort by index descending
        const descIndexResults = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'index', sortDirection: SortDirection.Descending });
        expect(descIndexResults.length).to.equal(testVals.length);
        expect(descIndexResults).eql([...testVals].reverse());
      });
      ...
      it('sorts lexicographic with and without a cursor', async () => {
        ...
        // sort ascending with cursor
        const ascResultsCursor = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: 'b' });
        expect(ascResultsCursor.length).to.equal(2);
        expect(ascResultsCursor).to.eql(['c', 'd']);
        ...
        // sort descending with cursor
        const descResultsCursor = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', sortDirection: SortDirection.Descending, cursor: 'b' });
        expect(descResultsCursor.length).to.equal(1);
        expect(descResultsCursor).to.eql(['a']);
      });
      ...
      it('sorts numeric with and without a cursor', async () => {
        ...
        // sort ascending with a cursor
        const ascResultsCursor = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', cursor: '2' });
        expect(ascResultsCursor.length).to.equal(2);
        expect(ascResultsCursor).to.eql(['3', '4']);
        ...
        // sort descending with a cursor
        const descResultsCursor = await testIndex.filteredIndexQuery(tenant, [{ schema: 'schema' }], { sortProperty: 'val', sortDirection: SortDirection.Descending, cursor: '2' });
        expect(descResultsCursor.length).to.equal(4);
        expect(descResultsCursor).to.eql(['1', '0', '-1', '-2']);
      });
      ...
      it('sorts range queries with or without a cursor', async () => {
        ...
        // ascending without a cursor
        let response = await testIndex.filteredIndexQuery(tenant, [{
          letter: {
            gte : lowerBound,
            lte : upperBound
          },
        ...
        // descending without a cursor
        response = await testIndex.filteredIndexQuery(tenant, [{
          letter: {
            gte : lowerBound,
            lte : upperBound
          },
      ...
      it('sorts range queries negative integers with or without a cursor', async () => {
        ...
        const upperBound = 3;
        const lowerBound = -2;
        let results = await testIndex.filteredIndexQuery(tenant, [{
          digit: {
            gte : lowerBound,
            lte : upperBound
          }
        ...
        results = await testIndex.filteredIndexQuery(tenant, [{
          digit: {
            gte : lowerBound,
            lte : upperBound
          }
      ...
      it('supports multiple filtered queries', async () => {
        ...
        // query in ascending order.
        const results = await testIndex.filteredIndexQuery(tenant,[
          { digit: { gte: lowerBounds, lte: upperBounds } },
          { property: true }
        ], { sortProperty: 'val' });
        expect(results).to.eql([...compareResults], 'results ascending');
        ...
        // query in ascending order with cursor.
        const resultsWithCursor = await testIndex.filteredIndexQuery(tenant, [
          { digit: { gte: lowerBounds, lte: upperBounds } },
          { property: true },
        ], { sortProperty: 'val', cursor: cursor });
        expect(resultsWithCursor).to.eql([...compareResultsAfterCursor], 'results after cursor ascending');

        const descResults = await testIndex.filteredIndexQuery(tenant, [
          { digit: { gte: lowerBounds, lte: upperBounds } },
          { property: true }
        ], { sortProperty: 'val', sortDirection: SortDirection.Descending });
        expect(descResults).to.eql([...compareResults].reverse(), 'results descending');

        const descResultsAfterCursor = await testIndex.filteredIndexQuery(tenant, [
          { digit: { gte: lowerBounds, lte: upperBounds } },
          { property: true }
        ], { sortProperty: 'val', sortDirection: SortDirection.Descending, cursor: cursor });
