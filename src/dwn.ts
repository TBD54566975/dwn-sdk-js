...
import { EventsGetMessage, EventsGetReply, EventStream, EventStreamI, PermissionsGrantMessage, PermissionsRequestMessage, PermissionsRevokeMessage, ProtocolsConfigureMessage, ProtocolsQueryMessage, ProtocolsQueryReply, SubscriptionRequestMessage, SubscriptionRequestReply } from './index.js';
...
export class Dwn {
  ...
  /**
   * Handles a `Subscription Request` message.
   */
  public async handleSubscriptionRequest(
    tenant: string,
    message: SubscriptionRequestMessage): Promise<SubscriptionRequestReply> {
    const errorMessageReply =
        (await this.validateTenant(tenant)) ??
        (await this.validateMessageIntegrity(
          message,
          DwnInterfaceName.Subscriptions,
          DwnMethodName.Request
        ));
    if (errorMessageReply !== undefined) {
      return errorMessageReply;
    }
    const handler = new SubscriptionsRequestHandler(
      this.didResolver,
      this.messageStore,
      this.dataStore,
      this.eventStream as EventStream
    );
