...
export class ProtocolAuthorization {
  ...
  public static async authorize(
    ...
    const protocolDefinition = await ProtocolAuthorization
      .fetchProtocolDefinition(tenant, recordsWrite, messageStore);
    ...
    await ProtocolAuthorization.verifyActionCondition(
      tenant,
      recordsWrite,
      messageStore,
    );
  ...
  private static async fetchProtocolDefinition(
    ...
    const protocols = await messageStore.query(
      tenant,
      query,
    ) as ProtocolsConfigureMessage[];
  ...
  private static async constructAncestorMessageChain(
    ...
    while (currentParentId !== undefined) {
      ...
      const parentMessages = await messageStore.query(
        tenant,
        query,
  ...
  private static getRuleSet(
    ...
    let allowedRecordsAtCurrentLevel:
      | { [key: string]: ProtocolRuleSet }
      | undefined = protocolDefinition.records;
    let currentMessageIndex = 0;
    while (true) {
      const currentRecordSchema =
        messageChain[currentMessageIndex].descriptor.schema;
      const currentRecordType = recordSchemaToLabelMap.get(
        currentRecordSchema!,
      );

      if (currentRecordType === undefined) {
        throw new Error(
          `record with schema '${currentRecordSchema}' not allowed in protocol`,
        );
      }

      if (
        allowedRecordsAtCurrentLevel === undefined ||
        !(currentRecordType in allowedRecordsAtCurrentLevel)
      ) {
        throw new Error(
      ...
      allowedRecordsAtCurrentLevel =
        allowedRecordsAtCurrentLevel[currentRecordType].records!;
      currentMessageIndex++;
    }
  }
  ...
  private static verifyAllowedRequester(
    ...
    if (allowRule === undefined) {
      ...
      if (requesterDid !== tenant) {
        throw new Error(
          `no allow rule defined for requester, ${requesterDid} is unauthorized`,
        );
      }
      ...
      const expectedRequesterDid =
        messageForRecipientCheck.descriptor.recipient;
      ...
      if (requesterDid !== expectedRequesterDid) {
        throw new Error(
          `unexpected inbound message author: ${requesterDid}, expected ${expectedRequesterDid}`,
        );
      }
    } else {
  ...
  private static verifyAllowedActions(
    ...
    if (allowRule === undefined) {
      ...
      if (requesterDid === tenant) {
        ...
        throw new Error(
          `no allow rule defined for ${incomingMessageMethod}, ${requesterDid} is unauthorized`,
        );
      }
    }
    ...
    const inboundMessageAction =
      methodToAllowedActionMap[incomingMessageMethod];
  ...
  private static async verifyActionCondition(
    ...
    if (!isInitialWrite) {
      ...
      if (recordsWrite.author !== authorOfInitialWrite) {
        throw new Error(
          `author of incoming message '${recordsWrite.author}' must match to author of initial write '${authorOfInitialWrite}'`,
        );
      }
    }
  }
  ...
  private static getMessage(
    ...
    while (true) {
      ...
      const actualAncestorType = recordSchemaToLabelMap.get(
        ancestorMessage.descriptor.schema!,
      );
